...
uint8_t RCC_setCLKStatus(uint8_t CLK_TYPE ,uint8_t Status)
{
	...
	if ( CLK_TYPE == HSE )
	{
		if( Status == CLKON )
		{
			...
			while( (GET_BIT((RCC ->RCC_CR) ,CR_HSERDY) == 0 ) && (Timeout <= TimeoutMax) )
			{
				Timeout ++ ;
			}
	...
	if ( CLK_TYPE == HSI )
	{
		if( Status == CLKON )
		{
			...
			/*  Waiting until Clock is Ready or timeout is reached */
			while( GET_BIT((RCC ->RCC_CR) ,CR_HSIRDY) == 0  && Timeout <= TimeoutMax)
			{
				Timeout ++ ;
			}
			/* check if timeout is reached */
	...
	if ( CLK_TYPE == PLL )
	{
		if( Status == CLKON )
		{
			...
			/*  Waiting until Clock is Ready or timeout is reached */
			while( GET_BIT((RCC ->RCC_CR) ,CR_PLLRDY) == 0  && Timeout <= TimeoutMax)
			{
				Timeout ++ ;
			}
			/* check if timeout is reached */
